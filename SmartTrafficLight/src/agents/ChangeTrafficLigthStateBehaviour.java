...
public class ChangeTrafficLigthStateBehaviour extends CyclicBehaviour {
	...
	//Indice geral da direção em questçao, é assumido pelo menor índice em cada direção.
			...
			if (phase.getDuration() > 1) {
				...
				if (type == 1) {
					for (char c : phase.getState().toCharArray()) {
						// diego: por quê alterna apenas entre 'r' e 'y' ou 'g' e 'y' e faz isso em todas as fases?
